...
Dim iKCost1, iKCost2, iKCost3, iKCost4, dtTemp, sHref
...
Set RS1 = cn.Execute(sTemp)
	...
	Dim Str,SQL,RS,val,sTemp
		SQL = "SELECT ID, Description FROM tlkpLossSubCategories With (NOLOCK) WHERE LossCatID = 5 ORDER BY Description"
		Set RS=cn.execute(SQL)
		Str="<select name='"&Name&"' id=styleSmall>"&vbCRLF
...
RS1.Open "SELECT Count(*) AS RecCount FROM tblRIRoth with (NOLOCK) WHERE QPID=" & SafeNum(iQPID), cn
...
<form name="frmOth" method="post"   action="LOSSoth2.asp<%=sKey%>">
...
<table width="100%" border="1" cellPadding="2" width=100% cellSpacing="0">
	<tr>
	...
	<%				
	sTemp = ""
	sTemp = "SELECT * FROM tblRIRoth with (NOLOCK) WHERE QPID=" & SafeNum(iQPID) &" ORDER BY Seq"
	RS.Open sTemp, cn
	...
	<tr>
		<td align=center>
			<%If not bnew Then 
				sTemp = "<A class=em href=LOSSoth2.asp" & sKey & "&Delete=1&ID=" & RS("SEQ") & " onclick='return cmdDelete_onclick()'>" & iCntr & "</A>" 
			else
				sTemp= iCntr
			End If
		...
		<td align='center'>
			...
			<input type="text" name="txtQty<%=iCntr%>" size="2" value='<%=DisplayQuotes(sTemp)%>' maxlength="5">
			</td>
...
<% '+- OmniWorks Replacement History - qhse`quest`Rir:LOSSoth.asp;1 %>
