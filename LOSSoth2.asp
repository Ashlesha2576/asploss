...
Dim ACLDefined,MsgID,DelMsg
	...
	If Request.QueryString("Delete")=1 then 
		Set RS = conn.Execute("Select * from tblRIROth Where QPID="& SafeNum(iQPID) & " and Seq=" & SafeNum(trim(Request.QueryString("ID"))))
		If Not RS.EOF Then
			LogAuditTrail  iQPID,lOrgNo,dtRptdate,"R","LossOth2.asp",135,""	
			conn.Execute "DELETE FROM tblRIROth WHERE QPID="& SafeNum(iQPID) & "  AND Seq=" & SafeNum(trim(Request.QueryString("ID")))
		...
		conn.close
	    Set conn = Nothing
		Response.Redirect("LOSSoth.asp" & sKey & "&msg=" &Delmsg )
	end if
...
'WRITE PHASE		
	If bNotify = false Then	
		conn.execute "DELETE FROM tblRIRoth WHERE QPID="& SafeNum(iQPID) 
		...
		For iCtr = 1 to iCt			
			If bRowData(iCtr) Then
				iTemp = iCtr
				sTemp = "SELECT * FROM tblRIRoth WHERE QPID="& SafeNum(iQPID) & " AND Seq=" & SafeNum(iTemp)
				RS.Open sTemp, conn			
				If RS.EOF Then 
					RS.AddNew							
					 RS("QPID")		= iQPID
		...
		Next			
		' Costs
		UpdateCost conn, 5, iQPID, lOrgNo, dtRptDate
		UpdateUserInfo iQPID,conn
		conn.Close
		Set conn = Nothing
		LogAuditTrail  iQPID,lOrgNo,dtRptdate,"R","LossOth2.asp",MsgID,""
		Response.Redirect("LOSSoth.asp" & sKey & "&msg=" & server.URLEncode("Data Successfully Saved"))
	Else
		conn.Close
		Set conn = Nothing
	End if
...
<% '+- OmniWorks Replacement History - qhse`quest`Rir:LOSSoth2.asp;2 %>
